...
import type { CaseStudy, CaseStudyContentItem, CaseStudyResultSection } from "@/lib/caseStudies";
  ...
  return (
    <div>
      <div className="mb-4 flex items-center gap-4">
        <div className={`relative shrink-0 after:absolute after:left-1/2 after:top-1/2 after:-z-10 after:h-full after:w-full after:-translate-x-1/2 after:-translate-y-1/2 after:rounded-full after:blur-md ${toneClasses[tone].glow}`}>
          <Icon icon={icon} />
        </div>
      ...
      <div className="flex max-w-prose gap-5">
        <div className="w-9 shrink-0">
          <AnimationContainer direction="fromBottom" wrapperClass="h-full">
            <div className={`mx-auto h-full w-[3px] ${toneClasses[tone].line}`} />
          </AnimationContainer>
        </div>
...
export default function CaseStudyPage({ caseStudy }: CaseStudyPageProps) {
  const projectStats = caseStudy.projectStats.length > 0 ? caseStudy.projectStats : caseStudy.featuredKpis;
  const regularResults = caseStudy.results.filter((section) => section.visual !== "doughnutAB" && section.visual !== "chart");
  const restrictionResult = caseStudy.results.find((section) => section.visual === "doughnutAB" || section.visual === "chart");
    ...
    <Wrapper>
      <section className="w-screen translate-y-1 py-10 ml-[50%] -translate-x-1/2">
        <Wrapper>
          <div className="mb-16 flex flex-col justify-center gap-2">
            ...
            <h1 className="mb-5 text-center text-5xl font-bold uppercase">{caseStudy.name}</h1>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-8 md:flex-nowrap md:justify-between md:gap-1">
                ...
                <p className="text-xl">
                  ...
                  {caseStudy.website ? (
                    <>
                      {" "}
                      (
                      <a className="text-primaryAccent" href={caseStudy.website}>
            ...
            <Image className="my-auto rounded-full bg-white object-contain aspect-square" width={300} height={300} alt={`${caseStudy.name} logo`} src={caseStudy.logo} />
          </div>
        </Wrapper>
      </section>

      <section className="relative mx-auto mt-10 flex flex-wrap items-top justify-between gap-10 py-10 md:flex-nowrap">
        <DetailSection title="Challenges" items={caseStudy.challenges} tone="red" icon="warning" />
        <DetailSection title="Objectives" items={caseStudy.objectives} tone="green" icon="rocket" />
      </section>

      <section className="relative mx-auto mt-10 flex flex-wrap items-start justify-between gap-10 py-10 md:flex-nowrap">
        <DetailSection title="Initial Setup" items={caseStudy.initialSetup} tone="red" icon="tracking" />
        <DetailSection title="Solutions" items={caseStudy.solutions} tone="green" icon="audit" />
      </section>
              ...
              {projectStats.length > 0 ? (
                <div className="switcher mb-20 justify-center gap-5 md:gap-10">
                  {projectStats.map((kpi) => (
                    <KpiCounter key={kpi.title} beforeNumber={kpi.beforeNumber} number={kpi.number} afterNumber={kpi.afterNumber} title={kpi.title} />
                  ))}
                </div>
              ) : null}
      ...
      {caseStudy.testimonial ? (
        <section className="relative mt-10 py-10">
          <h2 className="mb-6 text-center text-5xl font-bold">Client Experience</h2>
          <p className="mx-auto mb-4 max-w-prose text-center text-2xl font-bold">"{caseStudy.testimonial.quote}"</p>
          {caseStudy.testimonial.name ? <p className="text-center italic">{caseStudy.testimonial.name}</p> : null}
          {caseStudy.testimonial.title ? <p className="text-center italic">{caseStudy.testimonial.title}</p> : null}
          <BackgroundLight />
        </section>
      ) : null}
